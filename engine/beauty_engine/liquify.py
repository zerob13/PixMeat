...
def build_face_handles(
    ...
    eye_enlarge = float(params.eye_enlarge) * confidence_scale
    if eye_enlarge != 0:
        for eye_indices in (idx.LEFT_EYE, idx.RIGHT_EYE):
            ...
            moved = eye_center + delta * (1.0 + eye_enlarge * 0.42)
            add_move(eye, moved)
            dynamic_indices.update(eye_indices)
            radius = max(8.0, float(np.ptp(eye[:, 0])) * 1.65)
            active_mask = np.maximum(
                active_mask,
                _radial_mask(shape, eye_center, radius, radius * 0.82) * min(1.0, abs(eye_enlarge) * 2.2),
            )

    nose_slim = float(params.nose_slim) * confidence_scale
    if nose_slim != 0:
        ...
        for row, point in enumerate(nose):
            ...
            moved[row, 0] += side * nose_slim * face_w * 0.072
        add_move(nose, moved)
        dynamic_indices.update(nose_indices)
        nose_center = np.mean(points[idx.NOSE_LEFT + idx.NOSE_RIGHT, :2], axis=0, dtype=np.float32)
        active_mask = np.maximum(
            active_mask,
            _radial_mask(shape, nose_center, face_w * 0.22, face_h * 0.24) * min(1.0, abs(nose_slim) * 2.5),
        )

    smile = float(params.smile) * confidence_scale
    if smile != 0:
        mouth_indices = list(dict.fromkeys(idx.OUTER_LIPS + idx.INNER_LIPS))
        ...
        for row, point in enumerate(mouth):
            ...
            moved[row, 0] += side * smile * face_w * 0.030 * corner_weight
            moved[row, 1] -= smile * face_h * 0.064 * corner_weight
        ...
        active_mask = np.maximum(
            active_mask,
            _radial_mask(shape, mouth_center, face_w * 0.30, face_h * 0.20) * min(1.0, abs(smile) * 2.5),
        )
